...
class JTNNVAE(nn.Module):
    ...
    def __init__(self, vocab, hidden_size, latent_size, cond_lnKD_size, cond_SelectPtoM_size, depthT, depthG, dropout_rate_GRU=0.0, dropout_rate_MLP=0.0):
        ...
        self.jtnn = JTNNEncoder(hidden_size, cond_lnKD_size, cond_SelectPtoM_size, depthT, nn.Embedding(vocab.size(), hidden_size))   
        self.decoder = JTNNDecoder(vocab, hidden_size, latent_size, cond_lnKD_size, cond_SelectPtoM_size, nn.Embedding(vocab.size(), hidden_size), dropout_rate_GRU=dropout_rate_GRU, dropout_rate_MLP=dropout_rate_MLP)
        
        self.jtmpn = JTMPN(hidden_size, depthG) 
        self.mpn = MPN(hidden_size, cond_lnKD_size, cond_SelectPtoM_size, depthG)

        self.A_assm = nn.Linear(latent_size + cond_lnKD_size + cond_SelectPtoM_size, hidden_size, bias=False)
    ...
    def encode(self, jtenc_holder, mpn_holder, jtenc_condition_holder, mpn_condition_holder):
        tree_vecs, tree_mess = self.jtnn(*jtenc_holder, *jtenc_condition_holder)
        mol_vecs = self.mpn(*mpn_holder, *mpn_condition_holder)
    ...
    def forward(self, x_batch, beta):   
        x_batch, x_jtenc_holder, x_mpn_holder, x_jtmpn_holder,x_jtenc_condition_holder, x_mpn_condition_holder, x_condition_holder = x_batch
        x_tree_vecs, x_tree_mess, x_mol_vecs = self.encode(x_jtenc_holder, x_mpn_holder, x_jtenc_condition_holder, x_mpn_condition_holder)
        z_tree_vecs,tree_kl = self.rsample(x_tree_vecs, self.T_mean, self.T_var)
        z_mol_vecs,mol_kl = self.rsample(x_mol_vecs, self.G_mean, self.G_var)
        
        cond_lnKD_batch, cond_SelectPtoM_batch = x_condition_holder
        z_tree_vecs = self.cat_z_cond(z_tree_vecs, cond_lnKD_batch, cond_SelectPtoM_batch)
        z_mol_vecs = self.cat_z_cond(z_mol_vecs, cond_lnKD_batch, cond_SelectPtoM_batch)
    ...
    def cat_z_cond(self, z, lnKD, SelectPtoM):

        if lnKD is not None:
            lnKD = create_var(lnKD)
            z = torch.cat((z, lnKD), dim=1)
        else:
            z = z

        if SelectPtoM is not None:
            SelectPtoM = create_var(SelectPtoM)
            z = torch.cat((z, SelectPtoM), dim=1)
        else:
            z = z
    ...
    def decode(self, x_tree_vecs, x_mol_vecs, cond_lnKD, cond_SelectPtoM, prob_decode, fast_uncertainty_decode=False):
        ...
        if cond_lnKD is not None:
            cond_lnKD_vec = torch.zeros((len(pred_nodes), 14))
            cond_lnKD_vec[:,cond_lnKD+14] = 1
        else:
            cond_lnKD_vec = None

        if cond_SelectPtoM is not None:
            cond_SelectPtoM_vec = torch.zeros((len(pred_nodes), 18))
            cond_SelectPtoM_vec[:,cond_SelectPtoM+18] = 1
        else:
            cond_SelectPtoM_vec = None

        jtenc_holder,mess_dict = JTNNEncoder.tensorize_nodes(pred_nodes, scope)
        condition_holder = cond_lnKD_vec, cond_SelectPtoM_vec
        jtnn = self.jtnn.cuda()
        _,tree_mess = jtnn(*jtenc_holder, *condition_holder)
        tree_mess = (tree_mess, mess_dict)
        A_assm = self.A_assm.cuda()
        x_mol_vecs = A_assm(x_mol_vecs).squeeze()
    ...
    def create_cond(self, cond, min_, length):
        cond = cond - min_
        cond_vec = torch.zeros(1, length)  # .cuda()
        cond_vec[:,cond] = 1
        return cond_vec
...
class JTNNVAE_prop(JTNNVAE):
    ...
    def __init__(self, vocab, hidden_size, latent_size, cond_lnKD_size, cond_SelectPtoM_size, depthT, depthG, prop, dropout_rate_GRU=0.0, dropout_rate_MLP=0.0, drop_prop_NN=0.0):
        super(JTNNVAE_prop, self).__init__(vocab, hidden_size, latent_size,cond_lnKD_size, cond_SelectPtoM_size, depthT, depthG, dropout_rate_GRU, dropout_rate_MLP)
        self.prop = prop
    ...
    def save_params(self, save_path, name_parameter_file):
        params = {
            'hidden_size': self.hidden_size, 
            'latent_size': int(self.latent_size * 2),
            'cond_lnKD_size': self.cond_lnKD_size,
            'cond_SelectPtoM_size': self.cond_SelectPtoM_size,
            'depthT': self.depthT, 
            'depthG': self.depthG, 
            'prop': self.prop,
            'dropout_rate_GRU': self.dropout_rate_GRU,
    ...
    def forward(self, x_batch, beta):
        x_batch, prop_batch = x_batch
        x_batch, x_jtenc_holder, x_mpn_holder, x_jtmpn_holder, \
            x_jtenc_condition_holder, x_mpn_condition_holder, x_condition_holder = x_batch
        ...
        # 简单统计得知 SA （合成难易程度）与 lnKD 和 SelectPtoM 没有相关性，所以在预测 prop (即 SA）时，不添加条件
        cond_lnKD_batch, cond_SelectPtoM_batch = x_condition_holder
        z_tree_vecs = self.cat_z_cond(z_tree_vecs, cond_lnKD_batch, cond_SelectPtoM_batch)
        z_mol_vecs = self.cat_z_cond(z_mol_vecs, cond_lnKD_batch, cond_SelectPtoM_batch)
    ...
    def sample_prior(self, cond_lnKD=None, cond_SelectPtoM=None, prob_decode=False):
        z_tree = torch.randn(1, self.latent_size).cuda()
        z_mol = torch.randn(1, self.latent_size).cuda()

        # get property
        all_vec = torch.cat([z_tree, z_mol], dim=1)
        prop_net = self.prop_net.cuda()
        prop_pred = prop_net(all_vec)

        if cond_lnKD is not None:
            lnKD_vec = self.create_cond(cond_lnKD, min_=-13, length=14).cuda()
            z_tree = torch.cat((z_tree, lnKD_vec), dim=1)
            z_mol = torch.cat((z_mol, lnKD_vec), dim=1)

        if cond_SelectPtoM is not None:
            SelectPtoM_vec = self.create_cond(cond_SelectPtoM, min_=-9, length=18).cuda()
            z_tree = torch.cat((z_tree, SelectPtoM_vec), dim=1)
            z_mol = torch.cat((z_mol, SelectPtoM_vec), dim=1)

        # get smile
        smile = self.decode(z_tree, z_mol, cond_lnKD, cond_SelectPtoM, prob_decode)

        return smile, prop_pred
