...
if __name__ == '__main__':
    ...
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True, collate_fn=lambda x:x)
    ...
    for epoch in range(MAX_EPOCH):
        ...
        word_acc, topo_acc, assm_acc, prop_acc, it = 0, 0, 0, 0, 0
        ...
        for batch in tqdm.tqdm(dataloader):
            ...
            x_batch = tensorize(batch_to_tensorize, vocab, cond_lnKD_size, cond_SelectPtoM_size, assm=True, type="pretrain")
            batch_to_model = x_batch, prop_batch

            loss, kl_div, wacc, tacc, sacc, pacc = model(batch_to_model, beta=0)
            loss.sum().backward()
            optimizer.step()
            ...
            if (it + 1) % PRINT_ITER == 0:
                ...
                iter_step = it + 1
                print("Iter: %d, KL: %.1f, Word: %.2f, Topo: %.2f, Assm: %.2f, Prop: %.4f" % (iter_step, kl_div,
                                                                                     word_acc, topo_acc, assm_acc, prop_acc))
                wandb.log({"KL": kl_div, "Word": word_acc, "Topo": topo_acc, "Assm": assm_acc, "prop": prop_acc})
                word_acc,topo_acc,assm_acc,prop_acc = 0,0,0,0
                sys.stdout.flush()
            it += 1
        scheduler.step()
        torch.save(model.state_dict(), args.save_path + "/model.pre_trained_epoch-" + str(epoch))
        print("learning rate: %.6f" % scheduler.get_last_lr()[0])
    
    torch.save(model.state_dict(), args.save_path + "/model.pre_trained_final")
